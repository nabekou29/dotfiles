...
When creating new rule files in `rules/`, **always include YAML frontmatter**:
...
- Include `paths:` when the rule should only load for specific files
- Omit `paths:` (but keep empty frontmatter `---\n---`) when the rule should always load
...
This "Applies to:" pattern in markdown body does NOT work as a path filter. Always use frontmatter `paths:` instead.
