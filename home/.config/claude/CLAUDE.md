...
コードコメント・コミットメッセージ・PR の説明文で、**確認していない意図を
推測で書かない**。「Why を書く」は、知らない Why を作る理由にならない。

- 根拠になるのはコード・設定・ドキュメント・実測結果・本人の発言のいずれか。
  どれにも無ければ、その Why は書かずに省く。事実だけ書くか、質問する。
- 周辺の事実から合成したもっともらしい説明が最も危ない。読み手は確定した
  事実として受け取り、後から誤りだと気づけない。
