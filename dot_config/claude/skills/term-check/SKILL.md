...
3. **概念一貫性チェック**(ここがあなたの仕事): 抽出サマリの識別子・コメント・
   ...
   - 定義なき新出概念(glossary に無く、inventory にも無い概念)
     レポートの新出ワード(2。英語)はここで「既存語の同義語か(→既存語を
     提案)」「本当に新概念か(→glossary 登録を提案)」を判断する。
     文脈が足りなければ該当ファイルを Read する
4. **新出日本語フレーズの一般性チェック**(ここもあなたの仕事): スクリプトが
   報告した新出日本語フレーズ(2b)の **全て** について「日本で一般的に通じる
   表現か」を検証する。英語の新出ワード(2)は手順 3 で扱うため、ここでは対象外:
   - 自分の知識で判定できるものは判定する。確立した技術用語、辞書語、
     業界で広く通用する表現は OK
   - 直訳調・造語・既存語の組み合わせで意味が変わっているものは NG とし、
     一般的に通用する表現を代替案として提示する
   - 判定に迷うものは Web 検索(利用可能な場合)で「<語> 意味」「<語> 技術用語」等を確認する
